...
function Contents() {
  ...
  return (
    <div className="p-5 px-6 bg-[var(--bg-color)]">
      <h1 className="mt-6 text-[var(--primary-color)] text-4xl">Skills</h1>
      ...
      <div key={e.id} className="flex flex-col justify-around p-2 items-center gap-2">
        ...
        <p className="font-bold text-[var(--primary-color)]">{e.name_skill}</p>
      </div>
      ))}
    
</div>      
      <h2 className="text-2xl font-semibold py-3 text-[var(--primary-color)]">
        Additional skills
      </h2>

      <ul className="text-[var(--primary-color)] my-5 grid lg:grid-cols-5 md:grid-cols-3 grid-cols-2">
        {addSkills.map(e=>(
